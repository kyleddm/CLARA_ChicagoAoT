    ...
    def search(self, 
        ...
        for idx in indices[0]:
            if idx != -1:  # faiss returns -1 for padded results if fewer than k are found                
                ...
                logger(self.log_file,f'FAISS:Retreived Metedata for index {str(idx)}:{metadata}')
                if filter_func is None:
                    result_metadata.append(metadata)
                elif filter_func(metadata):
                    filter_result=filter_func(metadata)
                    logger(self.log_file,f'Found match for {filter_result}!  Appending ')
                    result_metadata.append(filter_result)
                else:
                    logger(self.log_file,f'FAISS: No metadata found for filter, using closest found results.')
                    result_metadata.append(metadata)
            else:
                logger(self.log_file,f'FAISS: Padded results returned.  appending no metadata')
